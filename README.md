...
> Data-Loss-Prevention - Open source tool by AetherCodeHQ

`cli` `data-loss` `golang` `prevention` `security`

## What is it?

**Data Loss Prevention** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Data-Loss-Prevention --path ./target

# With options
./Data-Loss-Prevention --path ./target --format json --output report.json

# Verbose mode
./Data-Loss-Prevention --path ./target --verbose
```
...
### Basic scan
```bash
./Data-Loss-Prevention --path ./my-project
```

### JSON report
```bash
./Data-Loss-Prevention --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Data Loss Prevention
  run: ./Data-Loss-Prevention --path . --format json --output report.json
```
